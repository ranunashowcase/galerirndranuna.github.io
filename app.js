...
function handleFileChange(e) {
  ...
  dropzone.innerHTML = '<div class="flex flex-col items-center justify-center py-4"><p class="text-sm font-medium text-blue-600">Memproses gambar...</p></div>';
  ...
  reader.onload = function(ev) {
    ...
    img.onload = function() {
      ...
      canvas.toBlob(function(blob) {
        selectedFileData = blob;
        dropzone.classList.add('hidden');
        document.getElementById('uploadPreviewContainer').classList.remove('hidden');
        document.getElementById('uploadPreviewImage').src = URL.createObjectURL(blob);
...
async function handleSaveImage(e) {
  ...
  try {
    var fileName = activeCode + '/' + Date.now() + '.webp';
    var uploadRes = await sb.storage.from('rnd-gallery').upload(fileName, selectedFileData, { contentType: 'image/webp' });
